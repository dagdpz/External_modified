function data = TDTbin2mat_working(BLOCK_PATH, varargin)
...
if ~useOutsideHeaders
    ...
    while ~feof(tsq)
        ...
        for x = 1:numel(uniqueCodes)
            ...
            % selection of specific varNames
            if ismember(varName, DONTREAD);
                continue;
            end
            if ~isempty(EXCLUSIVELYREAD) && ~ismember(varName, EXCLUSIVELYREAD)
                continue;
            end
...
%             else
             if any(CHANNELS~=0) && ismember(varName,STREAMSWITHLIMITEDCHANNELS)
                
                % add CHANNELS selection here
                valid_idx=ismember(headerStruct.stores.(varName).chan,CHANNELS);
                headerStruct.stores.(varName).chan=headerStruct.stores.(varName).chan(valid_idx);
                headerStruct.stores.(varName).ts=headerStruct.stores.(varName).ts(valid_idx);
                headerStruct.stores.(varName).data=headerStruct.stores.(varName).data(valid_idx);
                if isfield(headerStruct.stores.(varName), 'sortcode')
                    headerStruct.stores.(varName).sortcode =headerStruct.stores.(varName).sortcode(valid_idx);
                end
            end
        end
...
%% debugging BROA Broa conflict at the very beginning
BROA_idx=ismember(storeNames,'BROA');
if any(BROA_idx)
storeNames(BROA_idx)={'Broa'};
headerStruct.stores.Broa=headerStruct.stores.BROA;
headerStruct.stores=rmfield(headerStruct.stores,'BROA');
BROA_idx=ismember({blockNotes(:).StoreName},'BROA');
blockNotes(BROA_idx).StoreName='Broa';
...
for ii = 1:numel(storeNames)
    ...
    elseif isequal(currentTypeStr, 'snips')
        ...
        if ~NODATA
            ...
            [headerStruct.stores.(currentName).ts, t_idx] = sort(headerStruct.stores.(currentName).ts);
            headerStruct.stores.(currentName).chan = headerStruct.stores.(currentName).chan(t_idx);
            headerStruct.stores.(currentName).sortcode = headerStruct.stores.(currentName).sortcode(t_idx);
            %headerStruct.stores.(currentName).index = headerStruct.stores.(currentName)(ind);
            headerStruct.stores.(currentName).data = headerStruct.stores.(currentName).data(t_idx,:);
        end
    elseif isequal(currentTypeStr, 'streams')
        ...
        for jj = 1:length(sevList)
            if any(strfind(sevList(jj).name, currentName)) %|| any(strfind(sevList(jj).name,[currentName(1) lower(currentName(2:end))])) %% unfortunate, temporary Broa BROA confusion
                useSEVs = 1;
                break
            end
        end
        ...
        if useSEVs
            ...
            if CHANNEL == 0 && ismember(currentName,STREAMSWITHLIMITEDCHANNELS)
                d = SEV2mat_working(BLOCK_PATH, 'EVENTNAME', currentName, 'CHANNELS', CHANNELS, 'VERBOSE', 0, 'RANGES', validTimeRange);                
            elseif CHANNEL == 0 
                d = SEV2mat_working(BLOCK_PATH, 'EVENTNAME', currentName, 'VERBOSE', 0, 'RANGES', validTimeRange);
            else
                d = SEV2mat_working(BLOCK_PATH, 'EVENTNAME', currentName, 'CHANNEL', CHANNEL, 'VERBOSE', 0, 'RANGES', validTimeRange);
            end
            ...
            for jj = 1:numRanges
                ...
                if CHANNEL > 0
                    ...
                    % small adjustment for CHANNELS to work
                    nchan = double(max(fc));
                ...
                for f = 1:numel(arr)
                    ...
                    for kk = 1:numel(relativeOffsets)
                        ...
                        if CHANNEL > 0
                            if isempty(find((ind(kk,end) <= numel(tevData)),1)) %isempty(find((ind(kk,:) <= numel(tevData)),1))
                                if ~foundEmpty
                            ...
                            foundEmpty = false;
                            headerStruct.stores.(currentName).data{jj}(1, chanIndex:(chanIndex + npts - 1)) = tevData(ind(kk,:))'; %% caused error here
                            chanIndex = chanIndex + npts;
                        else
                            if isempty(find((ind(kk,end) <= numel(tevData)),1)) % isempty(find((ind(kk,:) <= numel(tevData)),1))
                                if ~foundEmpty
