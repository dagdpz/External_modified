...
[b_regress,pINT_regress,R_regress,Rint_regress,stats_regress] = regress(ytmp,xtmp);

% check the presence of outliers
outl=find(ismember(sign(Rint_regress),[-1 1],'rows')==0);
% if ~isempty(outl) 
...
%         ytmp(outl)=[]; xtmp(outl,:)=[];
%         [p_regress,pINT_regress,R_regress,Rint_regress] = regress(ytmp,xtmp);
%     end
% end

xtmp(:,2)=[]; %delete column 2
%save coefficients value
m(1)=b_regress(1); q(1)=b_regress(2);
...
m(2)=(pINT_regress(3)-b_regress(1))/cv; %slope standard error
m=[m pINT_regress(1,:)]; %add slope 95% C.I.
q(2)=(pINT_regress(4)-b_regress(2))/cv; %intercept standard error
q=[q pINT_regress(2,:)]; %add intercept 95% C.I.
...
%Total Variability
ym=polyval(b_regress,xm);
vtot=sum((ytmp-ym).^2);

%Regression Variability
ystar=ytmp-R_regress;
vreg=sum((ystar-ym).^2);

%Residual Variability
vres=sum(R_regress.^2);
...
cir2=[ystar+cv*sy2 ystar-cv*sy2];
    ...
    %test on slope
    t_slope=abs(m(1)/m(2)); %Student's t
%     if pr(2)<1e-4
...
%     end
%     try
%         powerStudent(t_slope,n-1,2,0.05);
%     catch ME
%         disp(ME)
...
%     end
    %test on intercept
    t_intercept=abs(q(1)/q(2)); %Student's t
    p_intercept=(1-tcdf(t_intercept,n-2))*2; %p-value
%     powerStudent(t_intercept,n-1,2,0.05);
    %Power of regression
    Zrho=0.5*reallog((1+abs(r(1)))/(1-abs(r(1)))); %normalization of Pearson's correlation coefficient
    sZ=realsqrt(1/(n-3)); %std.dev of Zrho
    pwr_regression=1-tcdf(1.96-Zrho/sZ,n-2)*2; %power of regression
    %Test for homoschedasticity of residuals (Modified Levene's test)
    xme=nanmedian(xtmp);
    e1=R_regress(xtmp<=xme); me1=nanmedian(e1); d1=abs(e1-me1); dm1=mean(d1); l1=length(e1);
    e2=R_regress(xtmp>xme);  me2=nanmedian(e2); d2=abs(e2-me2); dm2=mean(d2); l2=length(e2);
    gl=(l1+l2-2); S2p=(sum((d1-dm1).^2)+sum((d2-dm2).^2))/gl;
    t_homoschedasticity=abs(dm1-dm2)/realsqrt(S2p*(1/l1+1/l2)); cv=tinv(0.975,gl); 
    p_homoschedasticity=(1-tcdf(t_homoschedasticity,gl)); %p-value
...
%     disp('Press a key to continue'); pause; disp(' ')
    ...
    subplot(1,2,2);
    xl=[min(x) max(x)];
    plot(xtmp,R_regress,'bo',xl,[0 0],'k-',xl,[RSE RSE],'g--',xl,[-RSE -RSE],'g--',...
...
switch nargout
    ...
    case 3
        ...
        STAT.r=r;
        STAT.t_slope=t_slope;
        STAT.t_p=pr(2);
        STAT.t_intercept=t_intercept;
        STAT.p_intercept=p_intercept;
        STAT.t_homoschedasticity=t_homoschedasticity;
        STAT.p_homoschedasticity=p_homoschedasticity;
        STAT.pwr_regression=pwr_regression;
        STAT.b_regress=b_regress;
        STAT.pint_regress=pINT_regress;
        STAT.r_regress=R_regress;
        STAT.rint_regress=Rint_regress;
        STAT.stats_regress=stats_regress; %  R-square statistic, the F statistic and p value for the full model, and an estimate of the error variance.
        STAT.Spearman_corrcoef=rs;
