function [output]=Ourexpregr(x,y,verbose, Outlier)
...
% this should be changed to 0 or 1 for Outliers!!!!  shit
if nargin==2
    verbose=1; Outlier=1;
else
...
%  [b,bint,r,rint] = regress(y,X) returns an n-by-2 matrix rint of intervals
% that can be used to diagnose outliers. If the interval rint(i,:) for 
% observation i does not contain zero, the corresponding residual is larger 
% than expected in 95% of new observations, suggesting an outlier.
%  
%  In a linear model, observed values of y are random variables, and so are 
% their residuals. Residuals have normal distributions with zero mean but 
% with different variances at different values of the predictors. To put 
% residuals on a comparable scale, they are "Studentized," that is, they are 
% divided by an estimate of their standard deviation that is independent of 
% their value. Studentized residuals have t distributions with known degrees 
% of freedom. The intervals returned in rint are shifts of the 95% confidence
% intervals of these t distributions, centered at the residuals.

%check the presence of outliers
outl=find(ismember(sign(Rint),[-1 1],'rows')==0);

if  ~isempty(outl)
    % shit uncommented 064062013 Danial
...
%     xme=nanmedian(xtmp);
%     e1=R(xtmp<=xme); me1=nanmedian(e1); d1=abs(e1-me1); dm1=mean(d1); l1=length(e1);
%     e2=R(xtmp>xme);  me2=nanmedian(e2); d2=abs(e2-me2); dm2=mean(d2); l2=length(e2);
...
%         if Outlier==1; hold on;  plot(outl, yt(outl), 'ro');   end
%     else       
%         hold on
...
%     %title(txt)
%     
%     text(5,0.85, 'Red dotted lines: 95% Confidence interval of regression', 'Color' , 'k')
%     text(5,0.81, 'Green dotted lines: 95% Confidence interval of new y evaluation using this regression', 'Color' , 'k')
%     text(5,0.77, 'red points are outliers', 'Color' , 'r')
%     if t>cv
%          text(5,0.73,['Test passed: slope= ' num2str(m(1)) ' (=/= 0)'], 'Color' , 'r') 
%      else
%          text(5,0.73,'Test not passed: slope = 0', 'Color' , 'g')
%      end
%     
%     %plot residuals
%     subplot(1,2,2);
...
%         STAT.sse=sum((xtmp-xm).^2);
%         STAT.r=r;
%         OUTL.name= [num2str(inputname(1)) '_outl'];
%         OUTL.elements=outl;
% end

%% OUTPUTS
        output.slope.value=m(1);
        output.slope.se=m(2);
        output.slope.lv=m(3);
        output.slope.uv=m(4);

        output.intercept.value=q(1);
        output.intercept.se=q(2);
        output.intercept.lv=q(3);
        output.intercept.uv=q(4);

        output.STAT.rse=RSE;
        output.STAT.cv=cv;
        output.STAT.n=n;
        output.STAT.xm=mean(x);
        output.STAT.ym=ym;
        output.STAT.sse=sum((xtmp-xm).^2);
        output.STAT.r=r;
        
        output.OUTL.name= [num2str(inputname(1)) '_outl'];
        output.OUTL.elements=outl';

        output.slopetest.t=t;
        output.slopetest.cv=cv;
        output.slopetest.P=pr(2);

        output.pearson.value= r(1);
        output.pearson.SE= r(2);
        output.pearson.LOV= r(3);
        output.pearson.UPV= r(4);
        output.pearson.Adjustd= r(5);
        
        output.spear_corrcoef= rs;
        
        output.figure.yt=yt;
        output.figure.xtmp=xtmp;
        output.figure.ystar=ystar;
        output.figure.cir=cir;
        output.figure.cir2=cir2;
        output.figure.Outlier=Outlier;
        output.figure.R=R;
