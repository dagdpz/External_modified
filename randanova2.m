...
Fdist = NaN(num,numel(groups));
% Calc first factor first
Frow1 = find(strcmp('X1',Table(:,1)));
Frow2 = find(strcmp('X2',Table(:,1)));
idx = grp2idx(groups{2});

nresid=numel(stats.resid);
resid=stats.resid;
if ~isnan(Factual(1,1))
    parfor p=1:num
        permm = zeros(nresid,1);
        ...
        Fcolumn = find(strcmp('F',Table(1,:)));
        if ~isempty(Table{Frow1,Fcolumn})
            Fdist(p,1) = cell2mat(Table(Frow1,Fcolumn));
        else
            Fdist(p,1) = NaN;
        end
    end
...
if ~isnan(Factual(2,1))
    parfor p=1:num
        ...
        [~,Table,~] = feval(fun,permm);
        Fcolumn = find(strcmp('F',Table(1,:)));
        if ~isempty(Table{Frow2,Fcolumn})
            Fdist(p,2) = cell2mat(Table(Frow2,Fcolumn));
        else
            Fdist(p,2) = NaN;
        end
