function [imageData, alpha] = export_fig_2014(varargin) %#ok<*STRCL1>
...
try
    ...
    if isvector(options)
        ...
        try
            ...
            print2eps(tmp_nam, fig, printArgs{:});
            ...
            if options.transparent
                ...
                %eps_maintainAlpha(fig, tmp_nam, origAlphaColors);
            end
    ...
    try
        for ar = 1:length(ars)
            if strcmp(ars(ar).Face.ColorType, 'truecoloralpha')
                ...
                %Find and replace - but return to non-alpha for other
                %objects that might follow (subplots f.e.)
                ix=findstr(fstrm,colorID);
                fstrm2 = strrep(fstrm, ...
                    sprintf(['CT\n' colorID ' RC\n']), ...
                    sprintf(['CT\n' originalColor ' RC\n' alpha_ ' .setopacityalpha true\nN\n']));
                
                ix=numel(fstrm2)-numel(fstrm)+ix(end)-2;
                fstrm=fstrm2;
