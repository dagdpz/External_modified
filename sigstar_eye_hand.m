function varargout=sigstar_eye_hand(groups,stats,nosort,temp_col)
...
for ii=1:length(groups)
	...
	H(ii,:)=makeBar(xlocs(ii,:),thisY,stats(ii),temp_col);
end
%-----------------------------------------------------
...
function H=makeBar(x,y,p,temp_col)
...
H(1)=plot(x(:),y,'Color',temp_col,'LineWidth',1.5);
...
H(2)=text(mean(x(:)),mean(y)+myRange(ylim)*offset,stars,...
   	...
   	'BackGroundColor','none','Color',temp_col);
